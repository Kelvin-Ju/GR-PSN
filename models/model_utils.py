...
def getInput(args, data):
    ...
    if args.in_mtrl: input_list.append(data['mtrl'])
    return input_list

def parseData(args, sample,timer=None, split='train',index=2):
    # print(type(sample),index)
    if index<=1:
        sample=sample[index]
    input, target, mask ,mtrl = sample['img'], sample['N'], sample['mask'] ,sample['mtrl']
    ...
    if args.in_mtrl :
        mtrl = sample['mtrl']
        if args.cuda : mtrl = mtrl.cuda()
        mtrl_var = torch.autograd.Variable(mtrl);
        data['mtrl'] = mtrl_var
    return data 
...
def saveCheckpoint(save_path, epoch=-1, model=None, optimizer=None, records=None, args=None,model_name=None):
    state   = {'state_dict': model.state_dict(), 'model': args.model}
    records = {'epoch': epoch, 'optimizer':optimizer.state_dict(), 'records': records, 
            'args': args}
    torch.save(state, os.path.join(save_path, f'{model_name}_checkp_%d.pth.tar' % (epoch)))
    torch.save(records, os.path.join(save_path, f'{model_name}_checkp_%d_rec.pth.tar' % (epoch)))
