    ...
    def saveIterRecord(self, epoch, reset=True):
        for s in self.iter_rec.keys(): # split
            ...
            for k in self.iter_rec[s].keys():
                ...
                self.records[s][k][epoch].append(torch.mean(torch.FloatTensor(self.iter_rec[s][k])))
        if reset: 
            self.resetIter()
    ...
    def iterRecToString(self, split, epoch):
        ...
        for c in classes:
            ...
            for k in self.iter_rec[split].keys():
                if (c in k.lower()):
                    strs += '{}: {:.3f}| '.format(k, torch.mean(torch.FloatTensor(self.iter_rec[split][k])))
            if strs != '':
                rec_strs += '\t [{}] {}\n'.format(c.upper(), strs)
        ...
        for c in classes:
            ...
            for k in self.records[split].keys():
                if (c in k.lower()) and (epoch in self.records[split][k].keys()):
                    strs += '{}: {:.3f}| '.format(k, torch.mean(torch.FloatTensor(self.records[split][k][epoch])))
            if strs != '':
                rec_strs += '\t [{}] {}\n'.format(c.upper(), strs)
        return rec_strs
